...
public enum ValidationError implements ErrorCodeInterface {
    ...
    FIELD_NO_SPACES(9041, "This field must have no spaces", HttpStatus.BAD_REQUEST),
    // Permit-specific validation errors (9042-9061)
    INVALID_STATE(9042, "Invalid state for this operation", HttpStatus.BAD_REQUEST),
    MAIN_PERMIT_ALREADY_EXISTS(9043, "Main environmental permit already exists for this business", HttpStatus.BAD_REQUEST),
    COMPONENT_PERMITS_EXIST(9044, "Cannot create main permit when component permits exist", HttpStatus.BAD_REQUEST),
    MAIN_PERMIT_EXISTS(9045, "Cannot create component permits when main permit exists", HttpStatus.BAD_REQUEST),
    INVALID_PERMIT_NUMBER(9046, "Invalid permit number format", HttpStatus.BAD_REQUEST),
    DUPLICATE_PERMIT_NUMBER(9047, "Permit number already exists", HttpStatus.BAD_REQUEST),
    INVALID_ISSUE_DATE(9048, "Issue date cannot be in the future", HttpStatus.BAD_REQUEST),
    INVALID_PERMIT_TYPE(9049, "Invalid permit type", HttpStatus.BAD_REQUEST),
    PERMIT_FILE_REQUIRED(9050, "Permit file is required", HttpStatus.BAD_REQUEST),
    INVALID_FILE_EXTENSION(9051, "Invalid file extension. Only PDF, JPG, JPEG, PNG are allowed", HttpStatus.BAD_REQUEST),
    FILE_SIZE_EXCEEDED(9052, "File size exceeds maximum allowed size (10MB)", HttpStatus.BAD_REQUEST),
    PERMIT_ALREADY_ACTIVE(9053, "Permit is already active", HttpStatus.BAD_REQUEST),
    PERMIT_ALREADY_INACTIVE(9054, "Permit is already inactive", HttpStatus.BAD_REQUEST),
    BATCH_SIZE_EXCEEDED(9055, "Batch size exceeds maximum allowed limit", HttpStatus.BAD_REQUEST),
    DUPLICATE_IN_BATCH(9056, "Duplicate permit numbers found in batch request", HttpStatus.BAD_REQUEST),
    INVALID_ISSUER_ORG(9057, "Invalid issuer organization", HttpStatus.BAD_REQUEST),
    INVALID_PROJECT_NAME(9058, "Invalid project name", HttpStatus.BAD_REQUEST),
    PERMIT_EXPIRED(9059, "Permit has expired", HttpStatus.BAD_REQUEST),
    NO_PERMITS_TO_DELETE(9060, "No permits found to delete", HttpStatus.BAD_REQUEST),
    CANNOT_DELETE_ACTIVE_PERMIT(9061, "Cannot delete active permit. Please deactivate first", HttpStatus.BAD_REQUEST),
    // Form-specific validation errors
    INVALID_QUESTION_TYPE(9062, "Question type is not valid", HttpStatus.BAD_REQUEST),
    DISPLAY_ORDER_ALREADY_EXISTS(9063, "displayOrder already used by another item.", HttpStatus.BAD_REQUEST),
    FORM_INACTIVE(9064, "Form is inactive.", HttpStatus.BAD_REQUEST),
