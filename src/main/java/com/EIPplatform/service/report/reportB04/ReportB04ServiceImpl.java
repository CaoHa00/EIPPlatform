...
public class ReportB04ServiceImpl implements ReportB04Service {
 // Dependencies inject qua constructor (final)
    ...
    @Override
    @Transactional
    public ReportB04 submitDraftToDatabase(UUID reportId, UUID userAccountId) {
    ...
    // save and update part 1
    private void saveOrUpdateReportInvestorDetail(ReportB04 report, ReportB04DraftDTO draftData) {
        ReportInvestorDetailDTO dto = draftData.getReportInvestorDetailDTO(); // Response DTO từ draft
        if (dto == null) {
            return;
        }

        ReportInvestorDetail entity;
        if (report.getReportInvestorDetail() != null && report.getReportInvestorDetail().getRidId() != null) {
            // Update: Merge partial từ DTO vào entity hiện có
            entity = report.getReportInvestorDetail();
            reportInvestorDetailMapper.updateEntityFromDto(dto, entity); // Method cho WasteWaterDataDTO
        } else {
