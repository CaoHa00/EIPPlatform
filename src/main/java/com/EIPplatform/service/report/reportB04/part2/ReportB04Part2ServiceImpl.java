...
public class ReportB04Part2ServiceImpl implements ReportB04Part2Service {
    ...
    @Override
    @Transactional
    public List<ProductCreationRequest> createReportB04Part2(UUID reportB04Id,
        ...
        List<Product> entities = productMapper.toEntityList(request.getProducts());
        ...
        // 5. Map → DTO list
        List<ProductCreationRequest> responseList = productMapper.toDTOList(savedList);
    ...
    public List<ProductCreationRequest> getReportB04Part2(UUID reportId, UUID businessDetailId) {
        ReportB04DraftDTO draft = reportCacheService.getDraftReport(reportId, businessDetailId);
        if (draft != null && draft.getProductDTOs() != null) {
            log.info("getProduct - reportId: {}, businessDetailId: {}", reportId, businessDetailId);
            return draft.getProductDTOs().getProducts();
        }

        log.warn("getProduct not found in cache - reportId: {}, businessDetailId: {}", reportId,
                businessDetailId);
        return null;
    }

    @Override // hàm này fetch auto lên các dữ liệu chỗ business detail đã có
    public List<ProductCreationRequest>  getInitialReportB04Part2(UUID reportId, UUID businessDetailId) {
        BusinessDetail businessDetail = businessDetailRepository.findById(businessDetailId)
                .orElseThrow(() -> exceptionFactory.createNotFoundException(
                        "BusinessDetail",
                        "id",
        ...
        List<ProductCreationRequest> productListDTOs = productMapper.toDTOList(businessDetail.getProducts());
