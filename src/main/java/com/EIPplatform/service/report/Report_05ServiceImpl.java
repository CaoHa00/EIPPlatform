...
import com.EIPplatform.mapper.report.ReportA_05Mapper;
...
public class Report_05ServiceImpl implements Report_05Service {

    private final ReportA05Repository reportRepository;

    private final BussinessDetailRepository businessDetailRepository;
    private final ReportA_05Mapper report_05Mapper;
    ...
    //     Report_05 report = reportMapper.toEntity(request, businessDetail, reportType, draftStatus, reportCode);
    ...
    //     Report_05 report = validateBusinessOwnership(reportId, currentUser);
    ...
    //     List<Report_05> reports = reportRepository.findWithFilters(
    //             businessDetailId,
    ...
    //     Report_05 report = validateBusinessOwnership(request.getReportId(), currentUser);
    ...
    //     List<Report_05> reports = reportRepository.findUpcomingReports(businessDetailId, startDate, endDate);
    ...
    //     Report_05 report = reportRepository.findById(reportId)
    ...
    // private Report_05 validateBusinessOwnership(UUID reportId, UserAccount currentUser) {
    //     Report_05 report = reportRepository.findById(reportId)
    //             .orElseThrow(() -> exceptionFactory.createNotFoundException("Report", "id", reportId, ReportError.REPORT_NOT_FOUND));
    ...
    // private void validateReportEditable(Report_05 report) {
    //     String statusCode = report.getReportStatus().getStatusCode();
    //     if (!"DRAFT".equals(statusCode) && !"REJECTED".equals(statusCode)) {
    ...
    //     Report_05 report = reportRepository.findById(request.getReportId())
    ...
    //     Report_05 originalReport = validateBusinessOwnership(reportId, currentUser);

    //     ReportStatus draftStatus = reportStatusRepository.findByStatusCode("DRAFT")
    //             .orElseThrow(() -> exceptionFactory.createNotFoundException("ReportStatus", "code", "DRAFT", ReportError.REPORT_STATUS_NOT_FOUND));

    //     Report_05 newVersion = Report_05.builder()
    ...
    //     Report_05 parentReport = report.getParentReport() != null ? report.getParentReport() : report;
    //     List<Report_05> versions = reportRepository.findByParentReportOrderByVersionDesc(parentReport);
