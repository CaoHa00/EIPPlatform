...
public interface EnvPermitsRepository extends JpaRepository<EnvPermits, Long> {
    ...
    List<EnvPermits> findByUserDetail(UserDetail userDetail);

    List<EnvPermits> findByUserDetailAndIsActive(
            UserDetail userDetail, Boolean isActive
    );

    @Query("SELECT ep FROM EnvPermits ep WHERE ep.userDetail.userDetailId = :userId " +
    ...
    @Query("SELECT COUNT(ep) FROM EnvPermits ep " +
            "WHERE ep.userDetail.userDetailId = :userId AND ep.isActive = true")
